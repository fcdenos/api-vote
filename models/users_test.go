...
func testUsersExists(t *testing.T) {
	...
	e, err := UserExists(ctx, tx, o.UUID)
	if err != nil {
...
func testUsersFind(t *testing.T) {
	...
	userFound, err := FindUser(ctx, tx, o.UUID)
	if err != nil {
		t.Error(err)
	}
...
var (
	userDBTypes = map[string]string{`UUID`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`, `FirstName`: `text`, `LastName`: `text`, `Email`: `text`, `Pass`: `text`, `VoteDone`: `boolean`}
	_           = bytes.MinRead
)
